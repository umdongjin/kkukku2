...
class ItemService {
  ...
  Future createNewItem(
      ItemModel2 itemModel, String itemKey, String userKey) async {
  ...
  Future<List<ItemModel2>> getUserItems(
      {required String userKey, String? itemKey}) async {
    CollectionReference<Map<String, dynamic>> collectionReference =
  ...
  Future<List<ItemModel2>> getNearByItemsGoogle(
      String userKey, LatLng latLng) async {
    // GeoFlutterFire is an open-source library that allows you to store
    // and query firestore documents based on their geographic location.
    final geo = Geoflutterfire();
    final itemCol = FirebaseFirestore.instance.collection(COL_ITEMS);
    ...
    for (var snapshot in snapshots) {
      ...
      itemModel.iconBytes =
          await getBytesFromNetwork(imgUrl: imgUrl, width: 48);
  ...
  Future<Uint8List> getBytesFromNetwork(
      {required String imgUrl, required int width}) async {
